    ...
    for (let step = 0; step < chunks.length; step++) {
        ...
        if (phase === 1) {
            ...
            if (time_left_in_minutes <= .2) {
                chunk_practical = chunk.slice(0, Math.min(Math.floor(CHUNK_SIZE/3), chunk.length))
            }
        }
...
async function init(exp_iteration, exp_id_prefix, mode, total_buckets) {
    ...
    var base_dir_to_save = `results_${COOL_DOWN_IN_MINUTES}/` + exp_iteration.toString();
...
async function init_mama(cool_down) {
    ...
    for(let i = 300; i < 500; i++) {
        ...
        if (LIVE) {
            exp_id_prefix = `live_zeus_${COOL_DOWN_IN_MINUTES}_` + i;
        }
        else {
